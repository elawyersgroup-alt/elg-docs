...
PENDING = [
    ...
    ("Брусилова: зона слабого подтопления 77:00-6.308 пропала из реестра 22.09 — решение за Игорем", "днём в 12:37 реестр отдавал обе зоны подтопления, вечером в 20:41 и 20:44 — только 77:00-6.311 (42 % участка); прямой поиск НСПД по номеру 6.308 отвечает 404, слой карты её не отдаёт. В абзаце «Что мешает» и в блоке ГПЗУ сказано про две зоны «как и в реестре границ» — пересборка сделает справку противоречивой, поэтому профиль и справка Брусилова оставлены на данных 12:37. Варианты: переписать абзац, подождать и перепроверить (может быть сбоем реестра) или сослаться на ГПЗУ, где обе зоны документированы", "wait"),
    ("Индекс актов от 22.09 покрывает меньше кадастровых номеров, чем индекс от 07.09", "ППТ 8 591 → 7 510 уникальных номеров, изъятие 36 334 → 36 199, КРТ 3 447 → 3 228; появилась категория «скан, не распознан (промежуточный индекс)» — 728 актов. На четырёх пилотах результат не изменился (те же находки), но для новых участков покрытие ниже прежнего — нужен отдельный разбор, почему часть сканов осталась нераспознанной", "wait"),
