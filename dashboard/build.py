...
PENDING = [
    ...
    ("VPN «по требованию» — снять один раз (Игорь): иначе окна не будет никогда", "Системные настройки → VPN → ⓘ у hidemyname.vpn → выключить «Подключаться по требованию», затем Cmd+Q приложение и scutil --nc stop. Туннель поднимается сам через час после любого отключения (проверено 22.09: приложение перезапустилось в 10:03, OnDemandEnabled TRUE). Ждут окна: профиль по Академика Ильюшина, индекс актов, каталог слоёв НСПД", "late"),
