...
PENDING = [
    ("Почтовый сторож видел не все письма — починено 22.09; уведомления о письмах по-прежнему нет", "в журнале каждые десять минут стояло «новых 0», хотя письма приходили: сторож искал письма поиском UNSEEN, а письмо, открытое в почте раньше его захода, для такого поиска не существует. Теперь письма берутся по номеру (UID) с запомненной верхней границей; на письмо старше шести часов автоответ не уйдёт даже при включённых ответах — защита от пачки ответов после потери состояния. Первый заход после правки: «новых 20, типовых 0, отправлено 0, режим журнал; писем в ящике 249». Заодно починен разбор адреса из перенесённой строки заголовка (elg-pzz f092923, b0f580e, выложено и сверено с репозиторием). Отдельно: о новом письме Игоря не уведомляет ничто — сторож на Маке смотрит заказы и заявки сайта, а не ящик; Telegram у сторожа не настроен (tg.conf нет). Нужно решение: уведомлять ли о письмах и что показывать — время и тему или только время", "wait"),
    ...
    ("Брусилова: решение Игоря 22.09 — оставить как было; схема построена на тех же данных", "содержание справки не тронуто: обе зоны подтопления, данные сбора 22.09 12:37. Схема нарисована по контурам из ответов реестра границ, сохранённых 02.09; каждый контур принят только после сверки доли пересечения с участком с долей живого сбора — 0,9999 и 0,4189 совпали точно; происхождение контуров написано на схеме и в подписи. Что известно о самой зоне: днём в 12:37 реестр отдавал обе зоны подтопления, вечером в 20:41 и 20:44 — только 77:00-6.311 (42 % участка); прямой поиск НСПД по номеру 6.308 отвечает 404, слой карты её не отдаёт. В абзаце «Что мешает» и в блоке ГПЗУ сказано про две зоны «как и в реестре границ» — пересборка сделает справку противоречивой, поэтому профиль и справка Брусилова оставлены на данных 12:37. Варианты: переписать абзац, подождать и перепроверить (может быть сбоем реестра) или сослаться на ГПЗУ, где обе зоны документированы — к этому стоит вернуться, когда реестр устоится", "go"),
