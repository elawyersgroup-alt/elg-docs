...
PENDING = [
    ("Пароль приложения Яндекса → data/smtp.conf: последний шаг к письму о заказе", "письмо через SMTP самого ящика собрано и выложено; связь с smtp.yandex.ru с хостинга есть (1,3 с), не хватает только пароля. Ящик info@el-group.com.ru живёт на Яндексе, не на Hostland. Яндекс ID → Безопасность → Пароли приложений → Почта; вписать второй строкой в data/smtp.conf в панели; сказать Code «готово» — проверит вход и сделает пробный заказ. Паролей Code не вводит", "late"),
