...
PENDING = [
    ('Приложение «Радар объекта»: сервер на согласование — Timeweb Cloud, Москва, MSK 50: 1 200 ₽/мес (1 080 при оплате за год) + копии до 300 ₽/мес', 'план — plany/PRILOZHENIE-radar-2026-09-27.md. От Игоря: «да» и оплата; домен app.argument-law.ru; почта уведомлений (у el-group.com.ru нет DKIM); для MAX — профиль ИП, договор и бот; для автоплатежей — письмо менеджеру ЮKassa. MAX мини-приложения поддерживает — через бота', 'wait'),
    ('Пакет 27.09, п. 3.1: цены подписки «Радар объекта» — в PRICE.md и на панели', '«Собственник» — 5 900 ₽ в год, до 3 объектов; «Бизнес» — 1 500 ₽ за объект в месяц, от 5; раздела «не продаётся до запуска» нет. Страницы «Радар объекта» на сайте ещё нет: в файле этапа 0 она с формой списка ожидания, а новые формы — только после политики. Вопрос Игорю: выкладывать страницу сейчас без формы (почта, Telegram, телефон, как Р5) или вместе с приложением?', 'wait'),
    ...
    ('Анонсы статьи о ВРИ — Telegram и MAX (один текст), VK, подпись Instagram: ждут правки Игоря', 'teksty/posty-vri-2026-09-26.md; каждая фраза — из статьи, таблица сверки в файле; карточка Instagram — после «да» на подпись', 'wait'),
    ('Главная: в «Публикациях» статья о ВРИ встала первой, ушла «Как читать уведомление о приостановлении» — оставить так?', 'в блоке три места; вернуть карточку вместо КРТ или 700-ПП — одним словом Игоря', 'wait'),
    ('Пункт меню для Р5 «Проверка ВРИ» — нужен?', 'сейчас на страницу ведут статья, разбор на сайте, «П. 22» и «Разборы»; в меню её нет', 'wait'),
