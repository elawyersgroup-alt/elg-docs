...
PENDING = [
    ('Решения Мосгорсуда по перечню 700-ПП: 361 решение, свыше 940 пунктов — формулировка для писем, поста MAX и страницы ждёт Игоря', 'счёт по 15 сообщениям ДЭПиР (март–сентябрь 2026) — scripts/depr_sudy_schet.py: 972 пары дело–пункт, нижняя граница 948; прежние «289 решений, около 760 пунктов» — ошибка разбора. Решения 2024–2026 годов, вступили в силу в 2026-м — «с марта по сентябрь суд признал» неточно', 'wait'),
    ...
    ('Статья о ВРИ: планы сведены (план Игоря + дополнения Code) — на утверждение', 'RABOTA-vri-pereocenka-snt-2026-09-26.md, раздел 1а; уточнение к п. 3 ст. 14.3 ЗК (установленный вид, а не фактическое использование); письма Росреестра и Минстроя — проверить по текстам', 'wait'),
    ...
    ('Группа кадастровой оценки: доступ к Росреестру решён, в фонде данных и НСПД группы нет', 'фонд данных ГКО Росреестра → НСПД /cadastral-price/search, поиск /api/geoportal/v2/search/cadastralPrice — стоимость, удельный показатель, назначение, этажи; группы нет (проба 26.09). Доступ — безголовый Chrome с корнем Минцифры по SPKI и PAC; curl — ca/ru_i_sistemnye.pem. Группа — в отчёте ГКО и в разъяснениях ГБУ (ст. 20 237-ФЗ); отчёт — только по слову Игоря', 'wait'),
