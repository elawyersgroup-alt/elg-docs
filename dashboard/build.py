...
PENDING = [
    ...
    ("«Диус»: раздел 1 справки написан на три четверти — один абзац ждёт формулировки Игоря", "manual_77_09_0004006_11.json → lawyer_summary, 4 абзаца (Режим / Что мешает / Чего не знаем / Что делать), черновик Игоря. Code снял пометку «[проверить]» в абзаце 1: высота 75 м и виды использования по проекту планировки № 981-ПП от 03.06.2022 уже были сверены раньше и лежат в этом же файле (construction_note, act_notes); двенадцать кодов ЕГРН построчно сверены с записью зоны — жилых нет. В абзаце 2 осталась пометка «[проверить юристу]» про «нельзя перевести в жильё без изменения самой санитарно-защитной зоны» — это решение Игоря, не факт из источника; spravka.py корректно отказывается собирать справку, пока пометка стоит (единственная ошибка верификатора при сборке). Письмо риелтору — финальный текст есть у Игоря, Code его не видел и не отправлял", "wait"),
