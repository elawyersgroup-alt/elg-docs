...
PENDING = [
    ...
    ('Почта radar@el-group.com.ru и DKIM — нужен вход Игоря в Яндекс 360 домена el-group.com.ru', 'под входом, открытым в Chrome, организации Яндекс 360 нет (предлагает создать новую — не нажималось); ящик создаёт Игорь, запись DKIM вносит Code в DNS у Hostland по присланному значению', 'wait'),
    ('Цели Метрики radar_pochta, radar_telegram, radar_telefon — создать?', 'вызовы целей уже стоят на кнопках страницы «Радар объекта»; без целей счётчик нажатий — UNKNOWN', 'wait'),
    ...
    ('Пакет 27.09, п. 3.2: политика и оферта Р5 — ждут текстов от Игоря «как своих»; новые формы до публикации политики не открываются', 'в пакете — проект дополнения политики (п. 2, 3.1–3.5, 7) и оферта Р5 с местами «[проверить юристу]»: уведомление Роскомнадзора по ст. 22 152-ФЗ, срок хранения, НДС, договоры поручения', 'wait'),
