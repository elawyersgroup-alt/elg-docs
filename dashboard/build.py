...
PENDING = [
    ("Письмо о заказе с сайта НЕ уходит: mail() на Hostland отключён", "sendmail_path = /usr/bin/false — письмо не передаётся никакому серверу; заказы с формы лежат только в data/orders.csv, столбец «письмо» = НЕ ОТПРАВЛЕНО. Варианты: SMTP ящика на Яндексе (пароль приложения заводит Игорь) или запрос в поддержку Hostland. Telegram с хостинга недоступен, SPF у argument-law.ru нет. Отчёт: otchety/forma-zakaza-2026-09-21.md", "late"),
    ("leads.csv: 50 строк с 30.08 по 20.09, писем по ним не было ни одного", "lead.php шлёт тем же mail(); 13 строк — за 18–20.09, после запуска кампании. Сколько из них живые люди — UNKNOWN, файл только посчитан. Просмотреть в панели Hostland: data/leads.csv", "late"),
    ("Сайт: форма заказа на обеих страницах, выгружена 21.09", "номер участка и контакт собирает сайт, после отправки — кнопка оплаты; бот остался второй дверью. Три проверки: строка пишется, honeypot отсекает, без согласия не отправляется; orders.csv снаружи 403. order.php на сервере совпадает с репозиторием по SHA-256", "go"),
    ("Метрика: цели order_form_g1a и order_form_g2 созданы", "ID 655141417 и 655147816, «совпадает»; pay_click_g1a / pay_click_g2 снова значат клик по кнопке оплаты. В вёрстке формы цель стоит в .finally() — считает и неудачную отправку; править или оставить — решение Игоря", "wait"),
