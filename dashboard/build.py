...
PENDING = [
    ('Витрина Градпрофиля — Брусилова, /primer-spravki.html: ждёт «утверждаю» по четырём абзацам и статуса запроса ИАИС ОГД', 'решение Игоря 24.09: «Диус» не называем (адрес страницы называл бы объект, а сведения о его застройщике — чужой бизнес на нашем сайте), витрина — городской участок с торгов на Брусилова, адрес без названия объекта. Страница собрана (данные 24.09 14:51, три отличия от клиентской справки: нет «Что делать», пустого «Заказчика», служебной строки), лежит в рабочей копии elg-site, не выложена. По ходу исправлено: зоны подтопления ушли из реестра, и разделы 1, 6, 7 справки перестали с этим сходиться — перенесены в «зоны по ГПЗУ, которых нет в реестре» с пометкой, под каким номером значились 22.09. Выкладка ждёт двух ответов Игоря: «утверждаю» по абзацам 1, 2, 4, 5 раздела 1 (переписаны Code 22.09, без явного утверждения; до него справка не собирается — проверка 11) и статуса запроса в ИАИС ОГД от 07.09 в кабинете mos.ru. Абзац 3 о зонах подтопления — вариант «а», утверждён Игорем 24.09, дата из даты сбора', 'wait'),
    ('Правило: текст Claude — «предложено Claude» до явного «утверждаю» Игоря; в справке это проверка 11', 'решение Игоря 24.09 после того, как абзац о подтоплении, пришедший вставкой, панель назвала «абзацем Игоря». У каждого абзаца юриста теперь отмечено, чей он (author «Игорь» или «Claude», для Claude — дата «утверждаю»); справка с текстом Claude без утверждения не собирается. По записям: Михельсона и Колокольников — тексты Игоря 21.09; Брусилова — абзацы 1, 2, 4, 5 со словами Code; «Диус» — абзацы 1 и 2 со словами Claude. Две формулировки на панели, прежде приписанные Игорю, исправлены. Дата-факт в абзаце юриста ({{дата_реестра}}) подставляется из даты сбора, а утверждение абзаца о реестре сверяется с данными (elg-pzz 6d960f6)', 'go'),
    ...
    ('Статья о 700-ПП: абзац о сроке возврата — ждёт решения Игоря; ключевые слова исправлены', '24.09 Игорь: отрасль права оставить («Гражданское право» стоит), ключевые слова заменить, п. 3 ст. 79 перепроверить. Ключевые слова: поле Закон.ру считает не больше пяти слов, а не фраз, поэтому при публикации из десяти фраз сохранилась одна; теперь стоит «700-ПП, трёхлетний срок возврата налога», текст статьи при сохранении не изменился — сверено по отпечатку. П. 3 ст. 79 НК в действующей редакции — о поручении в казначейство, трёхлетнего срока в нём нет; суды цитируют его в редакции до 2023 года. Определение СКЭС ВС РФ от 12.03.2025 № 305-ЭС24-17676 прочитано целиком: срок идёт с окончания налогового периода, но не ранее срока подачи декларации (за 2019 год — с 30.03.2020), а не с решения об исключении из перечня. Постановление АС МО от 17.06.2025 — предприниматель, налог по уведомлению, срок со дня уплаты за период. Главный тезис статьи эти акты подтверждают; неточны «со дня уплаты» для организаций и ссылка на п. 3 ст. 79. Абзац Игоря («практика единообразно не ответила…») не поставлен: по его условию — только если пересказы точны, а они неточны. Игорю предложена другая формулировка, ждём ответа', 'wait'),
    ('Сайт сверен и отправлен на переобход', '24.09 по просьбе Игоря: 74 файла живого сайта (58 страниц, карта сайта, robots.txt, картинки, файлы) побайтно совпали с репозиторием, все отвечают 200, несуществующая страница — 404. Заголовки не длиннее 65 знаков, описания не длиннее 160, canonical свои, битых ссылок нет, логотип файлом на всех 55 страницах из карты. Яндекс.Вебмастер принял 55 адресов в переобход (в очереди; на сегодня осталось 95 из 150). Google: карта сайта отправлена повторно, запрошено индексирование двух новых разборов. После правки абзаца о сроке страницу о 700-ПП отправить ещё раз', 'go'),
    ...
    ("Брусилова: справка дополнена абзацем о зонах подтопления (предложен Claude); оговорка о реестре — в шаблоны и оферту", "23.09: в справку вписан абзац о зонах подтопления, предложенный Claude, — Игорь переслал его без явного «утверждаю», а панель ошибочно называла его абзацем Игоря (исправлено 24.09; 24.09 Игорь утвердил новую редакцию, вариант «а», с датой из даты сбора). Содержание абзаца 23.09: на 23.09 обе зоны в реестре не отображаются, в ГПЗУ от 08.07.2024 указаны обе, исключение из реестра не значит, что территория перестала быть подтопляемой, перед проектированием запросить у Московско-Окского БВУ актуальные сведения. Блок ГПЗУ датирован 22.09. Справка пересобрана, в «Загрузках». По решению Игоря строка «сведения реестра границ приведены на дату и время запроса; реестр изменяется, в том числе исключением и заменой зон» — в оговорках шаблона справки (раздел 13 — в справке оговорки там, раздел 12 — «Что не удалось установить») и отчёта Г1а, и в пункте 7 оферты на обеих страницах; сайт сверен по SHA-256. Канон, правило 18: факт — за сутки исчезли две зоны на 42 % площади без видимой замены", "go"),
