...
PENDING = [
    ('Сайт на телефоне: главная Игоря и правка 60 страниц проверены — ждут согласия на выкладку', 'главная на 390 px 11 574 → 7 138 px, компьютерная версия без изменений; после правки кнопок меньше 44 px и прокрутки вбок нет ни на одной странице; мельче 15 px — только метки прописными. Смартфоны — 55,8 % живых посетителей за месяц (Метрика). Цели mbar_* — создать в Метрике по слову Игоря', 'wait'),
    ...
    ('Статья о ВРИ: черновик Игоря сверен по первоисточникам — правка ждёт Игоря', 'teksty/statya-vri-2026-09-26.md: в определении ВС 305-ЭС23-10880 нет ЕГРН (это КС 42-П) — фраза исправлена; оба письма сами оговаривают ненормативность; лимит 25 % — подп. 4 п. 3.3.5 ПЗЗ; действующая редакция ПЗЗ не сверена — пометка оставлена, нужен официальный текст книги 1', 'wait'),
    ...
    ('Отчёт ГКО Москвы № 1/2025 — на флешке: группы 4 и 6 названы прямо, но присваиваются по тем же источникам, что перечень', 'фонд данных ГКО (НСПД), реестр 42902: отчёт — 5 томов, 74 МБ, скачан, MD5 совпали; приложения — 8,4 ГБ, качаются в фоне в /Volumes/SANDISK/pzz/raw/gko_moskva_2025 (FAT32 — архив 5,7 ГБ частями). Табл. 2-4 тома 1: осмотр не проводился; источники вида использования — суд, комиссия, исправление ошибок, декларация, информация ДЭПР, акты 217-ПП и 257-ПП, потом ЕГРН. Группа 4 — 101 104 объекта, группа 6 — 89 219, подгруппы 18.2 и 18.3. Дальше — группа по каждому объекту из архива и выборка по округу', 'wait'),
