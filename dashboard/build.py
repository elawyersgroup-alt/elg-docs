...
PENDING = [
    ...
    ("Метрика: старую цель «Оплата экспресс-разбора» перевести с «содержит» на «совпадает»", "сейчас «содержит: pay_click» ловит и pay_click_g1a, и pay_click_g2 — завышает цель и путает стратегию Директа; пять новых целей созданы 18.09 (ID 637674216…637675540)", "late"),
    ...
    ("Директ: переключить Chrome на логин с рекламным кабинетом", "сейчас direct.yandex.ru открывает регистрацию нового аккаунта (активен info@el-group.com.ru); после переключения Code собирает черновик, запуск и бюджет — только Игорь", "late"),
