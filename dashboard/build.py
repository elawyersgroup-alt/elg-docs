...
PENDING = [
    ("Пароль приложения Яндекса → data/smtp.conf: последний шаг к письму о заказе", "письмо через SMTP самого ящика собрано и выложено; связь с smtp.yandex.ru с хостинга есть (1,3 с), не хватает только пароля. Ящик info@el-group.com.ru живёт на Яндексе, не на Hostland. Яндекс ID → Безопасность → Пароли приложений: включить переключатель, создать для «Почты»; в почте нажать «Сохранить» на странице «Почтовые программы» (галочка IMAP уже стоит); вписать пароль второй строкой в data/smtp.conf (правой кнопкой → «Редактировать файл»); сказать Code «готово». Пароль начинает действовать через 2–3 часа — отказ сразу после создания не поломка. Обычный пароль от ящика на хостинг не класть: он от всего аккаунта с Директом. Паролей Code не вводит", "late"),
    ...
    ("leads.csv сегодня: из 50 строк людьми Метрика считает 27, из последних 13 — 9", "писать всем подряд нельзя: робот мог вписать чужой адрес. Какие строки люди — по файлу не видно (UNKNOWN); глазами или сопоставлением с Вебвизором по слову Игоря. Файл — в панели: data/leads.csv, новые внизу. Отчёт: otchety/uvedomleniya-o-zakazah-2026-09-21.md", "late"),
    ("До подтверждения канала живым заказом — orders.csv и leads.csv руками, утром и вечером", "решение Игоря 21.09; записано в RUNBOOK, раздел 0а. Ответ сайта «200» не значит, что Игорь о заказе узнал — смотреть столбец «письмо»", "wait"),
    ("Удалить шесть тестовых строк в orders.csv", "удаляет Игорь через панель; проще удалить файл целиком — order.php создаст его заново с заголовком. Сторож это переживает: узнаёт строки по отпечатку, а не по счёту", "wait"),
    ...
    ("Российское окно в рабочие часы: решить, как обеспечивается", "норматив «час» держится только при окне: новому участку нужен сбор с НСПД, 2–8 мин; с 18.09 окна не было ни разу. Либо VPN гасится по уведомлению о заказе, либо окно открыто постоянно", "late"),
    ("Окно на полчаса: индекс, профиль для витрины, очередь клиентов", "scripts/okno_rebuild.sh запускается до гашения VPN и ждёт сам; в очереди out/okno_queue.txt стоит адрес Академика Ильюшина, 21; индексу актов 21.09 исполнилось две недели", "late"),
    ...
    ("Вставить тексты: две услуги Яндекс.Бизнеса", "teksty/kanaly-2026-09-18.md раздел 3; кабинет у Игоря. В ЮKassa настройки «сообщение после оплаты» не существует — подробности в разделе 1 того же файла", "wait"),
    ...
    ("Директ: 28.09.2026 — точка решения по кампании", "смотреть pay_click_g1a и audit_to_g1a, а не клики; при нуле целей гасить, не досиживая до 04.10 (решение Игоря 18.09). С 21.09 к ним добавилась order_form_g1a — заявка с формы; сверять с числом строк в orders.csv", "wait"),
