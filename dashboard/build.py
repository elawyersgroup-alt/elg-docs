...
PENDING = [
    ...
    ('Письма 700-ПП: строка ИП во всех 30 письмах, редакция оснований утверждена; сверка ст. 378.2 — три уточнения ждут Игоря', '«ИП Еланский И. Ю. · ОГРНИП 323774600705065» — решение Игоря 26.09; пересобрано, самое длинное письмо — 93 % страницы; файлы в «Загрузках». Уточнения по п. 3, 4, 4.1 ст. 378.2 НК: «один из видов» ВРИ, «разрешённое использование» помещений, акт составляет ГБУ «МКМЦН» (п. 3.5 257-ПП) — раздел 8 «было — стало». Печать и отправка — Игорь, 06–08.10', 'wait'),
    ('Цены: Р5 «Проверка ВРИ» — 15 000 ₽, Р6 «Сверка к переоценке 2027» — 20 000 ₽ за объект; СНТ — без цены', 'решение Игоря 26.09, внесено в PRICE.md; зачёт Р5 — в Р3, Р6 — в исправление ЕГРН (номера у исправления в прайсе нет — уточнить); номера SKU — предложение Code', 'go'),
    ('Статья о ВРИ: план утверждён Игорем — текста плана у Code нет, запрошен', 'условие Игоря: наше дело — только после его исхода и с согласия клиента, до этого — «типовая ситуация»; статью — как можно раньше', 'wait'),
    ('216-ФЗ: переходной нормы нет — проверено в КонсультантПлюс', 'ст. 6: закон вступает в силу со дня опубликования, ст. 3 и 4 — с 01.03.2027; о договорах до этой даты закон молчит; на странице аудита тезиса «для договоров после 1 марта» нет', 'go'),
    ('Законопроект по СНТ «о ликвидации и воссоздании»: в СОЗД отдельного нет — вероятно, это № 1283945-8', 'поиск в СОЗД по ключевым словам 26.09; РГ 11.08.2026 — поправки депутатов ХМАО без номера; подтвердить по тексту законопроекта (PDF в СОЗД — скачивание с «да» Игоря)', 'wait'),
    ('257-ПП изменён 21.07.2026 (1966-ПП): здания без обследований — по группе кадастровой оценки помещений', 'п. 1.1(1).2: вид фактического использования — по группе и подгруппе кадастровой оценки нежилых помещений на 1 января налогового периода; срок обращения — 90 дней года включения (п. 3.9). К продукту Р6 и к текстам о 700-ПП', 'go'),
    ('700-ПП: «Сезонность», календарь, пять страниц и пост VK; ч. 4 ст. 37 ГрК на трёх страницах; уточнения оснований в письмах — «было — стало» ждёт Игоря', 'текст «Сезонности» Игорь прислал 26.09. Сверка с 257-ПП (ред. от 21.07.2026, КонсультантПлюс): сезона «сентябрь–октябрь» нет — обследования весь год (п. 3.10), сведения к перечню на следующий год — не позднее 1 ноября (п. 3.11), обращение о повторном обследовании — 90 календарных дней следующего года, комиссия и обследование — до 20 июня (п. 3.9), сведения до 1 июля уточняют перечень текущего года (п. 3.12). Все правки — teksty/BYLO-STALO-700pp-vri-2026-09-26.md; выкладка и Закон.ру — одной правкой после ответа Игоря', 'wait'),
