...
PENDING = [
    ...
    ("Решить: письмо клиенту после заказа со ссылкой на оплату", "написано из утверждённых фраз Игоря, с лимитами и выключателем; НЕ выложено — выкладку остановил защитный фильтр, Code его не обходил. Код в ветке pismo-klientu (elg-pzz). Текст письма — в otchety/avtomatizaciya-2026-09-21.md, раздел 3", "wait"),
    ...
    ("Яндекс Бизнес: новых продуктов в карточке нет", "в «Товарах и услугах» только старая линия; тексты двух услуг готовы с 18.09 (teksty/kanaly-2026-09-18.md, раздел 3). Code вставит по слову Игоря — это публикация. За месяц 74 просмотра профиля, 0 переходов на сайт, 1 отзыв", "wait"),
    ("Сайт: что улучшать в первую очередь", "1) посадочная кампании в двух шагах от формы заказа; 2) главная на первом экране продаёт только старую линию; 3) sem-proverok-pered-pokupkoy.html без входящих ссылок и без цели; 4) Яндекс 20.09 убрал из поиска 13 страниц серии osnovanie-p… как малоценные. Технически сайт чист: 53 страницы, битых ссылок нет, редиректы и HTTPS в порядке", "wait"),
    ("Директ 24–25.09: посмотреть поисковые запросы автотаргетинга", "он даёт 10 показов из 14; если запросы нецелевые — оставить категории «целевые» и «узкие». Баланс 5 608 ₽, хватит на 7 дней. К 28.09 наберутся единицы кликов — решение придётся принимать на малых числах", "wait"),
