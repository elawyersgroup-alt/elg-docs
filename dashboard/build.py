...
PENDING = [
    ("Ссылка ЮKassa на 2 900 ₽ (Г1а) и рабочее время для оферты", "страница проверки свёрстана и в elg-site (7016291); без этих двух подстановок не публикуется; затем 9 900 ₽ (Г2), позже 3 900 ₽ (Г1б)", "late"),
    ...
    ("Оферты Г1а и Г2: вписать рабочее время", "всё остальное дозаполнено по опубликованной оферте экспресс-разбора; проекты в teksty/", "wait"),
    ("Окно российской сети: три страницы из «54» и панель Hostland", "под VPN hidemy.name сайт и Hostland не отвечают; кандидат один — test-palette.html", "wait"),
    ("Сайт: после подстановок — проверить на телефоне и выгрузить на Hostland", "proverka-izyatie-krt.html, audit-pered-pokupkoy.html, sitemap.xml и все страницы после правки меню/подвала скриптом; цели Метрики создать в счётчике", "go"),
    ("Аккаунты на Авито и Яндекс Услугах", "тексты объявлений готовы (раздел 2 текстов 17.09)", "wait"),
    ("Десять риелторов: первое письмо", "текст готов (раздел 3), три первые проверки бесплатно", "wait"),
...
CANON = ["ничего не додумывать: не найдено — UNKNOWN, а не «нет ограничений»", "у каждого вывода [Certain] / [Likely] / [Speculation]", "выписка ЕГРН — источник истины, индекс актов — опережающий сигнал",
         ...
         "цены и ТЗ меняет только Игорь; в клиентских документах цен нет, названия — из config_price.json", "клиентские данные — только в приватном elg-pzz"]
