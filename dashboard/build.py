...
PENDING = [
    ("Запрос в поддержку GitHub: войти в GitHub в Chrome — пароль вводит Игорь, отправляет Code", "старые версии обоих публичных репозиториев после перезаписи ещё открываются по прямой ссылке (проверено 23.09: ответ 200). Текст запроса готов — elg-pzz/privacy/zapros-github-2026-09-23.md: без самих данных, только номера коммитов, 0 запросов на слияние, 0 ответвлений. Форма поддержки требует входа, а в Chrome входа в GitHub нет; пароль Code не вводит. Вкладка со входом открыта; после входа Code заполняет и отправляет", "wait"),
