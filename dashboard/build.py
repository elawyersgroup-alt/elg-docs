...
PENDING = [
    ("Выгрузить на Hostland две страницы — порядок заказа развёрнут", "коммит f521dda в elg-site: кнопки ведут в бота за номером участка, ссылку на оплату даёт бот; показы Директа уже идут, а живая кнопка ведёт по старому, рваному пути. Нужно слово «выгружай» или выгрузка Игорем", "late"),
    ("Ответить человеку в боте через «Диалоги»", "обращение от 19.09 без ответа третьи сутки: ответ Игоря ушёл боту как обычное сообщение и до адресата не дошёл; отвечать из кабинета PuzzleBot → Диалоги → пользователь → поле ввода", "late"),
    ("Нажать «Вернуть 2 900 ₽» в ЮKassa", "форма возврата тестового платежа 20.09 собрана до кнопки: позиция × 1, итого 2 900 ₽, причина вписана; перевод денег Code не выполняет", "late"),
    ...
    ("Директ: кампания № 714560547 «Проверка перед покупкой» — идут показы с 21.09", "модерация пройдена; 2 500 ₽ в неделю, срок до 04.10; цель pay_click_g1a после выгрузки означает клик «Заказать» → бот, а не → касса", "go"),
    ("Вычитать: оферта, автоответ бота, блок «Как это работает»", "оферта говорит «по ссылке платёжного сервиса на сайте», а ссылку теперь даёт бот; автоответ начинается с «проверка началась» до оплаты; блок шагов на двух страницах переставлен Code сверх задания", "wait"),
    ...
    ("Директ: 28.09.2026 — точка решения по кампании", "смотреть pay_click_g1a и audit_to_g1a, а не клики; при нуле целей гасить, не досиживая до 04.10 (решение Игоря 18.09)", "wait"),
