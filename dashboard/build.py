...
PENDING = [
    ...
    ("Метрика: цель «Оплата экспресс-разбора» заблокирована для правки", "остановка кампании № 713875754 не разблокировала форму (проверено); блокировка шире одной цели, причина не найдена — либо ждать, либо завести новую цель с «совпадает»", "wait"),
    ("Метрика: строгий фильтр роботов не включён", "необратимо стирает накопленные данные — отказ; по отчёту «Роботы» 46% событий сайта за месяц роботные, но пересчёт целей без роботов по источникам не снят", "wait"),
    ...
    ("Директ: черновик № 714560547 — прочитать перед запуском", "фразы и посадочная (audit-pered-pokupkoy.html) заменены на кластер «перед покупкой»; заголовки объявления и «Объект продвижения» кампании всё ещё про изъятие/КРТ — решение по тексту и запуск за Игорем", "wait"),
