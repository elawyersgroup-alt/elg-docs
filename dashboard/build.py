...
PENDING = [
    ('Сервер: три тестовых файла nagruzka_t9 в корзине — удаляет Игорь', '25.09 перенесены из www в папку argument-law.ru/korzina (вне сайта, /nagruzka_t9.php — 404, nagruzka.php работает как прежде). Окончательно удалять файлы Code не может — это правило, по которому Code работает, не запрет Игорю. Папка открыта в файловом менеджере Hostland — выделить три файла и «Удалить»', 'wait'),
