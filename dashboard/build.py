...
PENDING = [
    ...
    ('Статья о ВРИ: раздела 1а не было — Code написал проект тезисного плана, ждёт Игоря', 'RABOTA-vri-pereocenka-snt-2026-09-26.md, раздел 1а: нормы ст. 14.1–14.4 ЗК сверены; письма Росреестра от 21.04.2026 № 13-00288/26 и Минстроя — «проверить по текстам»; условие Игоря — наше дело только после исхода и с согласия клиента', 'wait'),
    ...
    ('1966-ПП: здания без обследований — в перечень по группе кадастровой оценки помещений; исправление группы уточняет перечень задним числом', 'прочитано в самом постановлении: п. 4.20(1) Положения о ДЭПиР, п. 1.1(1).2, 1.6.2 и 3.14 Порядка (257-ПП). Р6 — защита от попадания в перечень, ноябрьская волна писем по 700-ПП может быть больше. Перечень групп Департамента на mos.ru не найден; информационные сообщения ДЭПиР — .docx, скачивание — с «да» Игоря', 'wait'),
    ('700-ПП: «Сезонность» переписана и выложена (сайт и Закон.ру); календарь, четыре страницы, пост VK и ч. 4 ст. 37 ГрК — «было — стало» ждёт Игоря', 'раздел — целиком текст Игоря 26.09 (сроки 257-ПП, 863-ПП, ст. 383 НК в ред. 425-ФЗ), первый абзац — «внесудебное окно есть и после утверждения перечня»; сайт — elg-site 9b4924e, живой 82 из 82; Закон.ру, запись 119223 — отпечаток остального текста совпал, дата публикации прежняя. Разделы 2–7 «было — стало» (teksty/BYLO-STALO-700pp-vri-2026-09-26.md, копия .docx у Игоря) — на утверждении', 'wait'),
