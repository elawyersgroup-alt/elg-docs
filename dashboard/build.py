...
for line in price.splitlines():
    ...
    if len(c) >= 8 and re.match(r"^[ГРП]\d", c[0]):
        line_name = {"Г": "Град", "Р": "Росреестр", "П": "Подписка"}[c[0][0]]
...
plan_html = "" if not plan_dirs else (
    '<section class="card" id="plan"><p class="eyebrow">План продвижения · октябрь–декабрь 2026 · утверждён Игорем 25.09.2026</p><h2>Девять направлений</h2>'
...
PENDING = [
    ('Согласие на обработку исправлено 27.09 (ч. 1 ст. 9 152-ФЗ в ред. 156-ФЗ): заказ — одна галочка оферты, уведомления — отдельное согласие /soglasie-pd.html, реклама — третья', 'в журнал — версия оферты или согласия; п. 8 оферт Г1а и Г2 без согласия внутри оферты; отказ от услуги — в Р1 и Р5 (в Г1а и Г2 уже был, полный возврат); редакции от 27.09.2026; живой сайт 87 из 87. Форма чек-листа (скрытая отмеченная галочка) — ждёт решения Игоря: А — без почты, Б — отдельное согласие', 'wait'),
    ('Вторая партия писем пересобрана 27.09: вводная строка возвращена, комплексы — одно письмо на дом', 'Рочдельская 15, Нижняя Сыромятническая 11, Шмитовский 39 (по зданию 347 млн вместо 289); писем 30, резерв 14; все на одной странице (98 %); файлы 27.09 — «Загрузки»', 'go'),
    ('Ст. 47 ГрК: порога по стоимости для работы без СРО нет; прямого исключения для ИЖС нет — изыскания для ИЖС не обязательны (ч. 3 ст. 48)', 'ред. от 04.08.2026; ч. 2.1 — только для госпредприятий и организаций с долей публичных образований; СРО для добровольных изысканий по договору с застройщиком ИЖС — вопрос толкования [проверить юристу]', 'go'),
    ('Пиковая нагрузка по уведомлениям — сделана 27.09: «в течение двух рабочих дней» при пороге', 'nagruzka.php?p=upl, порог porog_upl (пока 10 — как у заказов проверки; число — Игорь), сторож сообщает раз в день', 'wait'),
    ('Приём уведомлений: три отличия от текста пакета — согласованы Игорем 27.09', 'title без бренда, description без «регистрации», зачёт — только в экспертизу того же объекта в течение 30 дней, как в оферте', 'go'),
    ('Телефон: нижняя панель — кнопки разными цветами, выложено 26.09 по замечанию Игоря', '«Позвонить» — зелёная со значком трубки, «Написать» — синяя со значком сообщения, «Проверить участок» — фиолетовая; до 380 px значок над надписью; 63 страницы по хэшу, живой сайт 85 из 85 (elg-site d94f06d)', 'go'),
    ('Договоры: шаблоны — elg-docs/dogovory/shabloni, подписанные — вне git (~/ELG-dogovory-podpisannye + флешка), в elg-pzz — только реестр', 'решение Игоря 27.09, п. 7; копия на флешку — задача приложения Claude «rezerv-dogovorov» каждый день в 21:05 (launchd macOS на флешку не пускает); договор с кадастровым инженером и поручение ПДн — на правке у Игоря (8 пометок «проверить»)', 'wait'),
    ('Анонсы статьи о ВРИ вышли 26.09 с правкой Игоря: Telegram 19:16, MAX 19:18, VK — запись № 20', 'тексты совпали посимвольно (teksty/posty-vri-2026-09-26.md); Instagram — карточка Игорю на просмотр, не опубликована', 'go'),
    ('Instagram: карточка к статье о ВРИ опубликована 27.09 в 00:29 по «да» Игоря', 'подпись совпала с утверждённой (669 знаков, отпечаток); публикаций 11 → 12, дубля нет', 'go'),
    ('Меню «Покупка и продажа» — «Проверка ВРИ перед арендой и сделкой» на всех страницах; строка на «Собственникам и инвесторам» — выложено 26.09', 'elg-site e58d8f7, 64 файла по хэшу, живой сайт 85 из 85', 'go'),
    ('Страница «Радар объекта» выложена 26.09 без формы: тарифы, «скоро», почта с темой «Радар объекта», Telegram, телефон; ссылка в подвале', 'поправки: заголовок до 65 знаков; вводные предложения о том, что Росреестр сообщает сам, не выложены — ч. 5 и 5.1 ст. 34 218-ФЗ: о зонах Росреестр уведомляет сам, «о сделках» нормы не найдено; счётчик спроса — в пятничном отчёте', 'go'),
    ('«Радар объекта»: вводный абзац — оставить нейтральным или Claude пишет сверенный (ч. 5 и 5.1 ст. 34 218-ФЗ)?', 'сейчас на странице — перечень Игоря со словом «Следим», без сравнения с государством', 'wait'),
    ('Сервер: «да» Игоря — Timeweb Cloud MSK 50, первый месяц помесячно; регистрация на ИП и оплата — Игорь', 'договор Timeweb для юрлиц и ИП (файл от 10.09.2026): п. 9.3 — оператор ПДн — клиент, п. 9.4 — «Договор не является поручением на обработку персональных данных» → в п. 3.5 политики облачный сервер не называть обработчиком по поручению; app.argument-law.ru — запись в DNS, когда будет адрес сервера', 'wait'),
    ('Почта el-group.com.ru: DKIM работает, DMARC p=none добавлен 27.09; ящик radar@ — Игорь', 'DMARC — режим наблюдения, без адреса отчётов (иначе ежедневные XML на info@); DKIM — «настроена верно»; ящик radar@ — Игорь (организация без тарифа)', 'wait'),
    ('Цели Метрики — созданы 27.09 все семь: radar_*, order_form_vri, upload_notice, home_to_foto, priost_to_foto', 'целевое событие, условие «совпадает»; номера целей 664089133, 664089238, 664089387, 664089518, 664089552, 664089596, 664089597; целей 29 из 200', 'go'),
    ('Вторая партия писем 700-ПП (список 1): 30 писем и 15 в резерве собраны 26.09 — файл и конверты в «Загрузках» до печати; 6 писем — в два комплекса (4 и 2 здания)', 'текст — раздел 1С TEKSTY (Игорь, 26.09); pisma_700pp.py --partiya2; адреса и индексы — ГАР, как в пилоте; отправка 6–8.10 — Игорь', 'wait'),
    ('Политика конфиденциальности (редакция от 26.09.2026) и оферта Р5 опубликованы; формы Р5 и приёма уведомлений открыты — 26.09', 'тексты Игоря дословно (сверка по абзацам: политика 7 из 7, оферта 12 из 12). Р5: форма на /proverka-vri.html, оферта #oferta, оплата — по ссылке после номера (как в оферте, п. 4), order.php принимает vri. Приём уведомлений: /uvedomlenie-foto.html и upload.php — JPG/PNG/PDF по содержимому, до 10 МБ и 5 файлов, закрытая папка (прямой адрес — 403), 5 отправок в час, письмо без вложений, удаление через 30 дней; проба 23:48 — файл дошёл, письмо отправлено, окно сторожа «ПРОБА»; ссылки — главная и 18 страниц о приостановке', 'go'),
    ('Канон, правило 25: цитата из судебного акта — только по полному тексту, с источником (Игорь 26.09)', 'повод — несуществующая цитата «независимо от указания в договоре аренды» в заметках к делу; заметки исправлены; в возражения до 13.10 — только формулировки сверки', 'go'),
    ('Сайт на телефоне выложен 26.09: главная Игоря, 60 страниц, метки обычными буквами, нижняя панель; цели mbar_* созданы', 'elg-site 0954001, 60 файлов по хэшу, живой сайт 82 из 82; на 390 px ни на одной странице нет кнопок меньше 44 px, прокрутки вбок и текста мельче 15 px; компьютерная версия та же. Цели: mbar_call 664029774, mbar_write 664029881, mbar_poisk 664029899', 'go'),
    ('Закон.ру: статья о 700-ПП приведена к редакции сайта — основания, 46-П, «акт Госинспекции» (26.09, по слову Игоря)', 'одной правкой; остальной текст сверен по абзацам — 27 из 27; дата публикации прежняя; анонс — «по акту о фактическом использовании»', 'go'),
    ('Решения Мосгорсуда: фраза — в письмах, посте MAX и на странице об исключении; пост в MAX опубликован 26.09 в 15:09', '«да» Игоря 26.09: 361 решение, свыше 940 пунктов (по 15 сообщениям ДЭПиР, scripts/depr_sudy_schet.py). Письма: все 30 на одной странице, поля 1,5 см, самое длинное — 97 %. Страница isklyuchenie-iz-perechnya-700-pp.html — elg-site d66d9db', 'go'),
    ('Главная: строка календаря по 257-ПП и полоса разбора «Уведомление о приостановлении регистрации в Росреестре уже пришло?» — выложены 26.09', 'строка «Сентябрь–октябрь» заменена на «В течение года — обследования объектов» (п. 3.10 257-ПП; elg-site 5853a00); полоса разбора — текст Игоря, вёрстка проверена на 1280/1024/768/390 (edb84ee); живой сайт 82 из 82, находок 0', 'go'),
    ('Письма 700-ПП: строка ИП и три уточнения оснований — во всех 30 письмах; печать и отправка — Игорь, 06–08.10', 'решения Игоря 26.09: «ИП Еланский И. Ю. · ОГРНИП 323774600705065»; «достаточно одного из видов», «разрешённое использование», акт «составляют ГБУ «МКМЦН» и Госинспекция» (сверено со ст. 378.2 НК и п. 3.5 257-ПП). Самое длинное письмо — 95 % страницы. Вопрос Игорю: у первого основания оставлен полный перечень целей (с общепитом и бытовым обслуживанием)', 'wait'),
    ('Цены: Р5 «Проверка ВРИ» — 15 000 ₽, Р6 «Сверка к переоценке 2027» — 20 000 ₽ за объект; оба засчитываются в Р3; СНТ — без цены', 'решения Игоря 26.09, PRICE.md; номера SKU — предложение Code', 'go'),
    ('Статья о ВРИ опубликована 26.09: Закон.ру (запись 119264) и разбор на сайте; страница Р5 выложена', 'текст на Закон.ру совпал с утверждённым (29 абзацев, отпечаток), метки и рубрики сохранились; /vri-zdaniya-norma-est-poryadka-net.html — ссылки с «Разборов», «П. 22», главной и Р5; Р5 /proverka-vri.html — две правки текста Игоря по сверке (регистратор; 25 % от площади всех зданий на участке); форма — после политики, order.php с продуктом «vri» готов, не выложен; переобход Вебмастера 18:26; живой сайт 84 из 84', 'go'),
    ('«С чем приходят к нам» на телефоне — компактный список, выложено 26.09', 'блок 1 145 → 508 px, главная 7 138 → 6 501 px; компьютерная версия без изменений; elg-site bdb472a', 'go'),
    ('Выборка ЦАО для ноябрьской волны: списки 1 и 2 собраны, связь «помещение → здание» — по адресу', 'список 1 — 1 276 зданий перечня в «неофисных» группах (зацепка Игоря подходит к ~790: 10.x, 7.8, 7.9); 2а — 2 076 «офисных» вне перечня (у 550 офисные помещения ≥ 20 % площади); 2б — 155 по 1966-ПП; связь по адресу [Likely], архив «Исходные данные» не скачивается; поправка: офисных помещений вне перечня 47 424, не 57 107. Предложение: 30 писем из списка 1 в неделю пилота — нужны «да» и текст письма', 'wait'),
    ('216-ФЗ: в аудите — консервативная позиция (запрет по дате регистрации права, не договора)', 'решение Игоря 26.09, записано в чек-лист и ТЗ (Г4) как наша позиция; опора сверена: ч. 6 ст. 2 159-ФЗ и ч. 5 ст. 57 218-ФЗ («одновременно с государственной регистрацией права»)', 'go'),
    ('Законопроекты по СНТ: № 1283945-8 прочитан — не о воссоздании; законопроект о ликвидации и воссоздании в СОЗД не найден', '1283945-8 (Дума ХМАО, 08.07.2026): регион сможет установить перечень документов, которыми подтверждается распределение участка после ликвидации СНТ (членская книжка, квитанции). Поправки о воссоздании товариществ (РГ, 11.08.2026) в СОЗД на 26.09 нет; мониторинг — 1-го числа', 'wait'),
    ('Отчёт ГКО Москвы № 1/2025: группы по объектам найдены; ЦАО — здания перечня на 76 % в «офисных» группах, помещения — на 93 %', 'группа — архив «Определение КС ОН», «3 Результаты группировки ОН»; «офисные» и «торговые» — 4, 6 и 18.1–18.3 (том 3, разд. 3.3.1; 18.4–18.8 — стоянки, склады, вспомогательные в МФК). Здания района 20\xa0037, из перечня 2026 года в отчёте 5\xa0347, в «офисных» группах 4\xa0071; помещения перечня 4\xa0969, в «офисных» 4\xa0628. Списки для писем — отдельной строкой', 'go'),
    ('700-ПП: «Сезонность», календарь, четыре страницы, ч. 4 ст. 37 ГрК — выложены 26.09; посты исправлены', '«было — стало» разделы 1–7 утверждены Игорем и выложены (elg-site 9b4924e, 784d2a8; живой 82 из 82); Закон.ру — «Сезонность» и абзац о пришедшем летом; VK пункт 5 и три анонса Клерка в Telegram исправлены, в VK и MAX анонс Клерка не редактируется; в MAX добавлены посты 22.09 и 23.09. Пост 24.09 для MAX — ждёт «да» Игоря. Снять «остаются недели» — задача на 25.11.2026', 'wait'),
    ('Главная, оболочка всех страниц и карта изъятия 2026 — выложены 26.09 по слову Игоря', 'одной выкладкой заменой файлов со сверкой SHA-256 (elg-site 044e5bf): 60 страниц, карта сайта и картинка карты (62 файла); живой сайт — 82 из 82, находок 0; главная и страница аудита — на переобходе в Вебмастере (12:25). На главной: объёмная карта участка, фактурный поиск, «С чем приходят к нам» в стиле orbital, таблица «магазины, бытовое обслуживание, культура, спорт», карточка календаря — цикл 2027 («До 28 мая», «Налог за год — до 28 марта следующего года. Сроки — по ст. 383 НК в редакции с 1 января 2027 года»), карта изъятия 2026 по округам — границы OpenStreetMap с подписью «© участники OpenStreetMap», подпись — «датированные с 1 января 2026 года». Меню «Услуги» не выходит за край экрана — на всех страницах', 'go'),
    ('Аудит перед покупкой: строка «Выкуп помещения у города по льготе» (216-ФЗ) выложена 26.09, формулировка — по закону', 'текст Игоря (teksty/audit-stroka-216fz.md); по решению Игоря 26.09 — «при рассрочке — с даты полной оплаты и погашения записи о залоге», как в ч. 6 ст. 2 Закона № 159-ФЗ в ред. 216-ФЗ; ст. 6 — с 01.03.2027; выложено со сверкой SHA-256; проверка — в ТЗ (Г4) с 01.03.2027', 'go'),
    ('КРТ 2026: решений в реестре документов mos.ru нет — не пробел сбора', 'в реестре за 2026 год 1 544 документа до 23.09 (681 постановление) — ни одного с «комплексного развития территории» или «планировки» в заголовке; 13 id отвечают 404; последнее решение о КРТ в реестре — 01.10.2025. Бесплатный поиск и проверка не затронуты. Есть ли решения о КРТ 2026 года в другом разделе mos.ru — не проверено', 'go'),
    ('Налог на имущество организаций с 2027 года: сроки меняются (425-ФЗ)', 'с налогового периода 2027 года по объектам с кадастровой базой налог исчисляет налоговый орган; налог — до 28 марта следующего года, авансы — до 28-го числа второго месяца после квартала (за I квартал 2027 — до 28 мая); за 2026 год — прежние сроки (28 февраля 2027, авансы — 28-го числа месяца после квартала). Основание — п. 118 и 120 ст. 2 Федерального закона от 28.11.2025 № 425-ФЗ (официальная публикация), абз. 2 п. 7 ст. 382 и п. 1 ст. 383 НК', 'go'),
    ('Дело об аренде помещения в гаражном комплексе: файлы в elg-pzz; устаревшие ссылки на ч. 4 ст. 37 ГрК — на трёх страницах сайта', 'исправленные версии от 26.09 — в elg-pzz (в elg-docs не кладутся). Сверено: глава II.1 ЗК введена 295-ФЗ, ч. 4 ст. 37 ГрК утратила силу с 01.03.2026. Ссылки на ч. 4 ст. 37 как на действующую — arenda-vri-uchastka-priostanovka.html, priostanovka-rosreestr.html, osnovanie-p22-naznachenie-i-vri.html; правка — после слова Игоря', 'wait'),
    ('Мониторинг: ГКО земель Московской области — по понедельникам, законопроекты по СНТ — 1-го числа', 'задачи gko-mo-proekt-otcheta и snt-zakonoproekty (работают, пока открыто приложение). ГКО МО на 26.09: проект отчёта «на стадии формирования» (сайт ГБУ МО «ЦКО»); при размещении — сразу Игорю, замечания — 30 дней', 'go'),
    ('Законопроекты по СНТ — статус на 26.09.2026', '№ 1283945-8 (изменение ст. 3 Закона о введении в действие ЗК — оформление участков после ликвидации СНТ): на рассмотрении, внесён Думой ХМАО — Югры 08.07.2026, 24.09.2026 — отзыв Правительства (СОЗД). Законопроект о ликвидации и воссоздании товариществ (внесён в июле): номер не установлен', 'wait'),
    ('Закон о регистрации с 23.01.2027: меняется ч. 4 ст. 32 (262-ФЗ)', 'Федеральный закон от 26.07.2026 № 262-ФЗ: документы для ЕГРН направляются в Росреестр не позднее 15 рабочих дней со дня решения об исключении ОКН из реестра — федерального значения (Правительство РФ) и, по-новому, регионального и местного значения (высший исполнительный орган субъекта); вступает через 180 дней после опубликования. Другие статьи 218-ФЗ на эту дату не просматривались', 'go'),
    ('Главная: схема участка без номеров — выложена 25.09', 'по задаче Игоря: та же справка, что и схема на primer-spravki.html (перерисовка с номерами совпала с ней побайтно); заголовок «Схема участка», в легенде нет кадастрового номера и реестрового номера зоны, адреса нет. Новый файл /img/samples/shema-glavnaya.png подключён на главной вместо primer-shema-uchastka.png, подпись и размер прежние, primer-spravki.html не тронута. Рисует karta.py с ключом --bez-nomerov: если в тексте остаётся номер или часть адреса, схема не рисуется. Выложено со сверкой SHA-256 (elg-site 6c5c952), живой сайт — 81 из 81', 'go'),
    ('Сайт: новая оболочка выложена 25.09 — главная, «О компании», три новые страницы и шапка с подвалом на остальных 55', 'по архиву Игоря sayt-novaya-obolochka-2026-09-26.zip и его согласию после скриншотов (1280 и 390, Chrome): главная, praktika, dlya-sobstvennikov, dlya-developerov, o-kompanii; абзац с цифрами — в редакции Игоря (индекс и поиск — по актам Москвы). До выкладки: якорь #oplata на экспресс-разборе, поиск принимает номер из ?n= и сразу проверяет, вопросы-ответы старой главной — на razbory.html (видимый блок и FAQPage), цели Метрики home_to_poisk, home_to_razbor, home_search. Три новые страницы — в карте сайта, 8 страниц — на переобход в Вебмастере. Затем шапка site-head, подвал site-foot, блок стилей SHELL и скрипт меню — на 55 страниц скриптом tools/obolochka.py (служебные файлы подтверждения прав не тронуты); проверено в Chrome: мобильное меню и выпадающие списки. Выложено заменой файлов со сверкой SHA-256 (elg-site 639e581, f4f8944), живой сайт — 80 из 80', 'go'),
    ('Индекс актов: сколько актов «с номерами» — правильный счёт', 'на 25.09 в индексе 5 471 акт по идентификатору mos.ru, из них с кадастровыми номерами 3 672 (раздел «Документы» 2 921, ДГИ 751), 46 321 номер. «4 690 актов» в отчётах 07–16.09 и в справке — это счёт скрипта отчётов по сочетаниям «вид, номер, дата» (сообщения ДГИ без номера за один день сливаются), а не акты с номерами; с номерами 16.09 было 3 649 — число не уменьшалось. В текстах о поиске по номеру — «свыше 3 600 актов с кадастровыми номерами»', 'go'),
    ('Радар за сентябрь: скачать акты сентября с mos.ru — нужно «да»', 'генератор сводки готов (elg-pzz/scripts/radar.py), образец за август в «Загрузках» — radar-obrazec-2026-08.docx: сообщения о планируемом изъятии первым блоком, по округам, вид, номер, дата, число участков, начало заголовка, ссылка на акт; без выводов. В индексе на 22.09 сентябрьских актов нет. Для сентября нужно скачать с mos.ru: 37 сообщений ДГИ о планируемом изъятии (106 файлов: 73 PDF и 33 DOCX, около 182 МБ) и 17 распоряжений об изъятии (17 PDF, 5,3 МБ) — на 25.09; после 30.09 — дозагрузка остального за сентябрь. Срок сводки — 06.10', 'wait'),
    ('700-ПП: пилот отобран 25.09 — 30 объектов и 15 в резерве, до срока 02.10', 'впервые включённые в перечень на 2026 год с учётом поправок, нежилые, кадастровая стоимость от 100 млн ₽ по НСПД, частная собственность, одно письмо на здание; порядок случайный и воспроизводимый (зерно 20261002). В пилоте 19 зданий и 11 помещений; стоимость от 109 млн до 8,4 млрд ₽, медиана 205 млн; объект в «Сколково» и три объекта без почтового индекса в ГАР заменены из резерва (решения Игоря 25.09); варианты писем А и Б — по 15; один объект — из дополнения 191-ПП. Налог посчитан двумя цифрами: для организации — 2 %, для гражданина — 1,5 %, свыше 300 млн — 2 %. Учёт — только elg-pzz/outreach/700pp-pilot.csv. Из 569 просмотренных номеров отпали: 483 — стоимость меньше 100 млн, 24 — государственная или неизвестная собственность, 14 — нет в НСПД, 3 — адрес или здание уже были', 'go'),
    ('700-ПП: 30 писем и лист конвертов пересобраны 26.09 — просмотр перед печатью и отправка (Игорь, 06–08.10)', 'в «Загрузках» и у Игоря: 700pp-pisma-pilot-2026-09-26.docx (А — 15, Б — 15) и 700pp-konverty-pilot-2026-09-26.docx. 25–26.09 по рецензии: повод «в перечне на 2025 год его не было» (проверено по всем 45 объектам), основания — отдельно для зданий (19 писем; п. 3, 4, 4.1 ст. 378.2 НК и 46-П в точной редакции, выбранной Игорем) и для помещений (11 писем, не 10: у одного помещения адрес в ГАР — на уровне здания), призыв с телефоном и почтой в А и Б; 26.09 — абзац о переоценке 2027 (текст Игоря; факты сверены: приказ ДГИ Москвы от 20.02.2026 № 55, п. 15 Порядка рассмотрения декларации). Шрифт 11, поля 2/1,5/1,8/1,8 см — каждое письмо на одной странице (самое длинное ~91 %). Строки ИП под подписью нет — решение Игоря 25.09', 'wait'),
    ('700-ПП: условия варианта Б (процент и срок) — Игорь, до первого ответа по варианту Б', 'в письме цифр нет; заготовка — 25 % от фактически полученной экономии и возврата за три года; в PRICE.md — только после утверждения', 'wait'),
    ('700-ПП: ставка 2,5 % для объектов дороже 300 млн — проверено, в Москве 2 %', 'НК РФ ст. 380 п. 1.3 (введён 176-ФЗ) — регионы устанавливают ставку для таких объектов, но не выше 2,5 %; закон Москвы № 64 в ред. от 26.11.2025 — 2,0 % «в 2023 году и последующие налоговые периоды», ступени для дорогих объектов нет; справочник ФНС по Москве за 2025 год — 2 %. Для граждан — закон № 51: 1,5 % по объектам из перечня, 2 % свыше 300 млн. Пересчёт не нужен', 'go'),
    ('Сервер: тестовые файлы nagruzka_t9 удалены — Игорь, 25.09', 'Code перенёс их из www в папку argument-law.ru/korzina (вне сайта) и открыл её в файловом менеджере Hostland; Игорь удалил. Проверено: korzina пуста, в www и www/data их нет, /nagruzka_t9.php — 404, nagruzka.php отвечает как прежде. Порядок на будущее: Code переносит в korzina и открывает вкладку, удаляет Игорь', 'go'),
    ('700-ПП: шесть поправок к перечням скачаны 25.09 и учтены', 'по «да» Игоря, официальные PDF с mos.ru, размеры совпали: 520-ПП, 1481-ПП, 2771-ПП — к перечню на 2025 год, 191-ПП, 863-ПП, 1838-ПП — к 2026 году. Главное: 191-ПП от 17.02.2026 дополнило перечень на 2026 год 259 объектами (146 зданий, 113 помещений) с действием с 1 января 2026 года; 201 из них в перечне на 2025 год не было, два из них потом исключило 1838-ПП; 863-ПП и 1838-ПП исключили из перечня на 2026 год 305 и 218 пунктов. Итог: номеров, впервые включённых в перечень на 2026 год, — 3 767, ещё 53 новых сняты поправками и в выборку не идут. Исключение в 2025 году не делает объект «впервые включённым» — такие в выборку тоже не берутся', 'go'),
    ('700-ПП: в письме две цифры налога — решение Игоря 25.09', 'для организации — 2 % от кадастровой стоимости (закон Москвы № 64, ст. 2); для гражданина — 1,5 % по объекту из перечня, а при стоимости свыше 300 млн — 2 % (закон № 51, ст. 1). Обе цифры посчитаны в outreach/700pp-pilot.csv (elg-pzz). Ставки сверены по тексту законов в редакции от 26.11.2025 (docs.cntd.ru)', 'go'),
    ('Директ: минус-слова добавлены 25.09 по решению Игоря', 'в кампанию № 714560547 добавлены минус-слова выписка, егрн, собственник, владелец, принадлежит, найти, посмотреть, местоположение и минус-фразы «кадастровая стоимость», «план участка», «кадастровый учет», «московской области»; сохранено, после перезагрузки в списке кампании 24 минус-фразы — 12 прежних и 12 новых. Основание — отчёт по поисковым запросам: за месяц такие запросы стоили 1 379,03 ₽ из 2 190,89 ₽. Эффект смотреть вместе с точкой решения 28.09', 'go'),
    ("Поиск на сайте: решения Игоря по мелочам — приняты и сделаны 25.09", "страница выложена с текстами плана как есть. Проверка сайта отмечает две вещи: заголовок 68 знаков (норма 65) и описание 164 (норма 160); короткие варианты — «Проверка на изъятие и КРТ по кадастровому номеру — бесплатно» (60) и описание без «для государственных нужд». Сверх утверждённых текстов Code добавил: пометку «— номер прочитан со скана акта» у упоминаний, найденных в распознанных сканах (как в платном отчёте), и служебные строки «Проверяем…», «Слишком много проверок с вашего адреса. Попробуйте через час.», «Поиск временно недоступен». Номера 50: поиск не проверяет, как сказано в плане, хотя в индексе 7 968 упоминаний номеров 50: из актов Москвы (Новая Москва). Решено 25.09: короткие заголовок и описание — на сайте; пометка «со скана» и служебные строки приняты; номера 50: — ищутся; цели Метрики заведены (poisk_check, poisk_order) после того, как Игорь дал учётной записи права на редактирование; ссылки — после проверки 50:", "go"),
    ("Бесплатный поиск по кадастровому номеру выложен: /poisk-izyatie-krt.html", "25.09 ночью, до срока 09.10. Выгрузка из индекса только «номер — акт — дата» (40 918 номеров Москвы, индекс на 22.09.2026), только точные совпадения, как в платной проверке; близких совпадений распознавания нет. Ответ — по одному номеру, 10 запросов в час на адрес, honeypot, журнал без адреса и без номера; выгрузка и журнал закрыты от веба (403). Проверено вживую: найден по тексту и по скану, не найден с датой, «не Москва», неверный формат, лимит — 429, кнопка заказа ведёт в форму проверки с уже вписанным номером. Живой сайт 77 из 77, Яндекс — в очереди на переобход. Следом 25.09: номера 50: (Новая Москва) ищутся и пускаются к оплате с предупреждением, ссылки с главной и со страницы проверки выложены, цели Метрики заведены, правило пиковой нагрузки работает — при 10 заказах проверки за день форма до оплаты показывает увеличенный срок, Игорю — уведомление; проверено без настоящих заказов. Осталось — анонс 12.10 (Игорь)", "go"),
    ("Оферты проверки и Градпрофиля: пункт о сроке при большой загрузке — на сайте, редакция от 25.09.2026", "Игорь 25.09: в оферту проверки — в редакции от 25.09 (увеличенный срок — до конца следующего рабочего дня), в оферту Градпрофиля — три рабочих дня; последняя фраза сохранена — заказ, оплаченный до сообщения, выполняется в обычный срок. Текст пункта написал Code, утвердил Игорь. Текст формы Градпрофиля при пиковой нагрузке утверждён Игорем: «Из-за большого числа заказов справка придёт до конца третьего рабочего дня. Если срок не подходит — напишите нам до оплаты». Счёт заказов и показ срока на форме — к выкладке поиска (план, раздел 3). Выложено заменой файлов со сверкой SHA-256 (elg-site fe9585a), живой сайт — 76 из 76", "go"),
    ...
    ('«Диус»: раздел 1 утверждён, справка собрана — не публикуем, лежит в приватном репозитории', 'решение Игоря 24.09: публичная страница о строящемся объекте, который продаётся, со сведениями о праве застройщика на участок — не тот спор, который нам нужен. Справка пригодится, если ответит риелтор. Раздел 1 утверждён целиком: абзац 2 — формулировка, которую Игорь прислал 22.09; абзац 1 «Режим» — финальная редакция, которую Игорь прислал 24.09, с двумя правками по его выбору после сверки с данными: «у нас нет» вместо «по участку нет» (у застройщика разрешение на строительство 2021 года, а к нему по закону прикладывается ГПЗУ — план был, у нас его нет) и «около 290 на 35 метров» вместо «310 на 35» (по контуру участка в реестре; 310 × 35 не сходится с площадью). Остальные факты абзаца совпали с данными. Справка собрана на данных реестра 24.09, верификатор прошёл; файл — в «Загрузках»', 'go'),
    ("Календарь мероприятий снят — пока не актуально (Игорь, 24.09)", "вечером 24.09 Игорь: «календарь мероприятий удали весь, пока не актуально». Удалены KALENDAR.md, раздел «Календарь мероприятий» и плитка ближайшего срока на панели, оба файла .ics (в репозитории и в «Загрузках» — в корзину). Сроки по мероприятиям из плана сняты. Вернуть — из истории git (elg-docs 955076c): код панели оставлен и сам соберёт раздел и .ics, если файл вернуть. Исследование с контактами организаторов осталось в elg-pzz", "go"),
    ("Telegram: лишний пост о 700-ПП удалён (Игорь, 24.09)", "24.09 в 12:42 в @rosreestr_iznutri вышли две одинаковые записи; вечером Игорь удалил лишнюю. Return отправил пост, но в ленте он появился позже, чем через 4 секунды; проверка показала ноль, и Code нажал отправку ещё раз. Удалять сообщения Code не может — удалил Игорь. Правило записано: после отправки ждать не меньше 15 секунд и дважды пересчитывать записи", "go"),
    ('Статья о 700-ПП: абзац о сроке возврата исправлен 25.09 по полному тексту определения ВС — на Закон.ру и на сайте', '24.09 абзац заменён текстом Игоря; Code до замены прочитал определение СКЭС ВС РФ от 12.03.2025 № 305-ЭС24-17676 целиком и предупреждал, что формулировка «практика единообразно не ответила… признала срок не пропущенным» подталкивает к мысли, будто ВС считал срок с решения суда. 25.09 Claude в чате это подтвердил (абзац писался по пересказам), Игорь решил: исправленный абзац без последней фразы — «решение суда об исключении срок не запускает; расходятся в начальной точке: АС МО — со дня уплаты, ВС — с окончания периода, но не ранее срока декларации». Оба акта сверены по полным текстам: в деле АС МО (А40-141003/2024) здание тоже исключено из перечня 700-ПП. На Закон.ру заменён только этот абзац — остальной текст совпал по отпечатку, дата публикации прежняя (24.09, 12:05), ключевые слова «700-ПП, трёхлетний срок возврата налога»; на сайте — со сверкой SHA-256 (elg-site fe9585a)', 'go'),
    ...
    ("ИАИС ОГД: запрос подан повторно 24.09 — ответ ждём к 01–02.10; квитанцию оплатить сразу", "первый запрос от 07.09 без ответа: по регламенту 5 рабочих дней, срок вышел 14.09. 24.09 около 23:00 Игорь подал тот же запрос повторно. Регистрация — до одного рабочего дня, срок — 5 рабочих дней с регистрации; без оплаты квитанции — приостановка и отказ (п. 2.9.3). Придёт ответ — сверить параметры со справкой по Брусилова, снять пометку об ожидании, пересобрать справку и витрину; абзац 4 раздела 1 меняется текстом, который утвердит Игорь. Без ответа Г3 не выпускается", "wait"),
    ...
    ("Статус запроса в ИАИС ОГД от 07.09 подтверждён: ответа нет (Игорь, 24.09)", "Игорь 24.09: ответа нет, фраза «запрос подан 7 сентября, ответ пока не получен» в абзаце по Брусилова верна. В тот же вечер запрос подан повторно", "go"),
    ...
    ("Абзацы юриста по Брусилова утверждены 24.09", "абзацы 1, 2 и 4 — «в том виде, как на снимках витрины»; абзац 3 о зонах подтопления — вариант «а» с датой из даты сбора; «Что делать» — текст Игоря, три абзаца. Справка пересобрана, в «Загрузках»", "go"),
...
<div class="cols">
 <div style="display:grid;gap:20px;min-width:0">
  ...
  <section class="card"><p class="eyebrow">Подписка «Радар объекта» — приложение</p><h2>Цены утверждены 27.09, выпуск — с приложением</h2><div class="scroll">{price_table("Подписка")}</div>
  <p class="meta" style="margin:10px 0 0">Оплата: «Собственник» — годовая через ЮKassa; «Бизнес» — счёт на год или квартал до автоплатежей. Бесплатно — один объект в приложении.</p></section>
 </div>
...
if events:
    g = ics_file("meropriyatiya-glavnoe.ics", [e for e in events if (e.get("топ-5") or rel_level(e.get("релевантность", "")) == "hi")
                                            and decision_chip(e.get("решение"))[1] != "не идём"], "ELG · мероприятия · главное")
