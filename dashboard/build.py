...
PENDING = [
    ...
    ("Проверить бота в Telegram: /start и автоответ на номер", "восемь старых кнопок в прежнем порядке, две новые девятой и десятой; отправить номер пилотного участка текстом и убедиться, что автоответ сработал — чинить до 21.09, а не после первого клиента", "late"),
    ("Тестовый платёж 2 900 ₽ своей картой и прогон пути заказа", "идёт сразу после проверки бота и НЕ ждёт пересборки индекса: тест меряет путь от кнопки до PDF, а не свежесть данных; засечь время номер → PDF, строка в out/zakazy.csv с пометкой «тест», платёж вернуть", "late"),
    ...
    ("Директ: кампания № 714560547 на модерации, показы с 21.09.2026", "запущена и оплачена Игорём 18.09; баланс 5 608 ₽ при потолке 2 500 ₽ в неделю покрывает срок до 04.10; «Объект продвижения» решено оставить как есть", "go"),
    ...
    ("Индекс актов: 21.09.2026 исполняется две недели", "последний в очереди, тест платежа его не ждёт; нужно российское окно — скрипт scripts/okno_rebuild.sh запущен и ловит окно сам, гасит VPN Игорь", "wait"),
