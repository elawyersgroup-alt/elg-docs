...
PENDING = [
    ("Оферта: пункт о сроке при большой загрузке — проект Code для обеих оферт, утверждает Игорь", "план продвижения, раздел 3: правило пиковой нагрузки — при 10 заказах проверки за рабочий день форма до оплаты показывает срок «до конца следующего рабочего дня», Игорю — уведомление; порог и текст меняет только Игорь. Проект пункта написал Code (подпись «Claude» от 24.09 исправлена 25.09): для оферты проверки, п. 5 «Срок», и вариант для Градпрофиля, п. 5 «Срок и порядок». Последняя фраза — заказы, оплаченные до сообщения, выполняются в обычный срок — остаётся обязательно (Claude в чате). Для Градпрофиля — три рабочих дня (Code заметил, что «до конца следующего рабочего дня» запаса не даёт; три дня предложил Claude в чате 25.09). Текст — plany/TEKSTY-prodvizhenie-2026-09-25.md, раздел 7. На сайт — после того, как Игорь пришлёт текст как свой", "wait"),
