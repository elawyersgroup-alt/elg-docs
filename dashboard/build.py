...
PENDING = [
    ...
    ("Удалить data/orders.csv и data/orders-6col-do-21-09.csv — за Игорем", "оба пробные; Code на сервере не удаляет. Панель Hostland → Файлы / FTP → Файловый менеджер → data → правой кнопкой по файлу → Удалить. orders.csv создастся заново при первом заказе, сторож это переживёт. В leads.csv три пробные строки Code: № 51–53 (proba-…)", "wait"),
