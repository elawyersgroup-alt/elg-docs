...
PENDING = [
    ...
    ("Письмо о скачанном чек-листе тоже доходит: 3 секунды", "lead.php переведён на ту же отправку, что order.php (запрет снят Игорем 21.09): пробная заявка 12:20:24 — письмо во «Входящих» 12:20:27. Тема «Чек-лист скачан: адрес», адрес человека стоит в «Ответить». Исходник теперь под git, формат leads.csv не менялся", "go"),
    ...
    ("Заявки на чек-лист — почти сплошь боты: писать по leads.csv, скорее всего, некому", "48 из 50 адресов одного шаблона: ровно 8 знаков, буквы с цифрами, без гласных, все на Яндексе; визиты круглые сутки, отказов 0 %, рекламных переходов нет. Девять «людей» по Метрике из последних тринадцати — тот же шаблон. Проверить можно, открыв столбец адресов. Решить: писать ли им и помечать ли такие заявки в теме письма. Отчёт: otchety/leads-boty-2026-09-21.md", "late"),
    ...
    ("Удалить data/orders.csv целиком — за Игорем", "Code на сервере не удаляет. Панель Hostland → Файлы / FTP → Файловый менеджер → data → правой кнопкой по orders.csv → Удалить; файл создастся заново при первом заказе, сторож это переживёт. Там же можно убрать orders-6col-do-21-09.csv. В leads.csv осталась одна пробная строка № 51", "wait"),
