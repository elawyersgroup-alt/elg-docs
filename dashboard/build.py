...
PENDING = [
    ...
    ("Метрика: цель «Оплата экспресс-разбора» заблокирована для правки", "все поля формы неактивны (вероятно, цель в стратегии кампании); варианты: остановить кампанию или завести новую цель с «совпадает»", "wait"),
    ...
    ("Счёт Директа: 108 ₽, кампания встанет завтра", "за месяц Директ дал 5 из 72 целевых действий (7%) при расходе 12 138,70 ₽ — бюджет не отработан; пополнять или дать встать — решение Игоря", "late"),
    ("Директ: черновик № 714560547 — заменить фразы на кластер «перед покупкой»", "прогноз: 6 290 показов против 1 065 у нынешних фраз — порог пройден; решение и запуск — Игорь", "wait"),
