...
PENDING = [
    ...
    ("Прочитать «Вывод юриста» в пересобранной справке по Брусилова: абзацы 1, 3, 4 переписаны по фактам ГПЗУ", "прежний текст говорил «параметры не подтверждаются» и «дождаться ИАИС ОГД или получить ГПЗУ» — с ГПЗУ в руках это устарело. Три фразы написаны Code: срок использования ГПЗУ три года (ч. 10 ст. 57.3 ГрК, до 08.07.2027); запросить ГПЗУ у продавца вместе с договором аренды; сверить с ответом ИАИС ОГД, что регламент с июля 2024 не менялся. Справка — в «Загрузках»", "late"),
    ("Решить: формулировка расхождения для ТЗ и страницы Градпрофиля", "предложена в отчёте, раздел 8: «+41 % по плотности и полная смена видов использования за семь лет — измерено на одном участке по документу органа». Слово «оценка» больше не нужно, слово «на одном участке» — нужно (канон, правило 7). ТЗ и страницу Code не меняет без слова", "wait"),
...
CANON = ["ничего не додумывать: не найдено — UNKNOWN, а не «нет ограничений»", "у каждого вывода [Certain] / [Likely] / [Speculation]", "выписка ЕГРН — источник истины, индекс актов — опережающий сигнал",
         ...
         "участок с торгов — сначала документы лота: ГПЗУ, выписка, условия; и только потом всё остальное (21.09.2026)", "рабочая пометка «[проверить…]» в тексте блокирует сборку справки"]
