...
PENDING = [
    ('Приём уведомлений: три отличия от текста пакета — на согласование', 'title без « — ELAWYERS GROUP» (68 знаков при пределе 65); description без слова «регистрации» (165 → 153 при пределе 160); «засчитывается в стоимость дальнейшей работы» → по оферте разбора: в стоимость правовой экспертизы того же объекта, если заказать её в течение 30 дней', 'wait'),
    ...
    ('Договоры — папка elg-pzz/dogovory/vhodyashchie: ждёт файлов от Игоря', 'Code заводит реестр: у каждого договора — услуга, оферта, цена и срок; расхождения с офертой и PRICE.md — отмечаются. Подписанные с клиентами — только в elg-pzz', 'wait'),
    ...
    ('Instagram: карточка к статье о ВРИ — на просмотр Игорю', '~/Downloads/instagram-vri-kvadrat-2026-09-26.png; подпись утверждена с правкой; публикация — после «да»', 'wait'),
    ...
    ('DKIM для el-group.com.ru работает; ящик radar@el-group.com.ru — Игорь', 'владелец организации Яндекс 360 — info@ (восстановлено 26.09 через DNS); запись mail._domainkey внесена у Hostland, Яндекс 26.09 ответил «DKIM-подпись настроена верно», статус домена — «Домен настроен». Ящик radar@ — Игорь (у организации нет тарифа — «ограниченная версия»). DMARC нет — вопрос Игорю', 'wait'),
    ('Цели Метрики — создать? radar_pochta, radar_telegram, radar_telefon, order_form_vri, upload_notice, home_to_foto, priost_to_foto', 'вызовы целей уже стоят на кнопках и формах; без целей в Метрике счётчики — UNKNOWN; создание целей — настройка аккаунта, только с «да» Игоря', 'wait'),
