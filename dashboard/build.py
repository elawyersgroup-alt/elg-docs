...
PENDING = [
    ...
    ("Зоны санитарной охраны: раздел 12 каждой справки — сделано; пункт оферты — вносит Игорь; слой ЗСО в НСПД — задача Code на первое окно", "в сохранённых ответах НСПД ни одного слоя ЗСО нет, ГПЗУ ссылается на них через СанПиН, а не через реестр границ — в ЕГРН зона не внесена [Likely]. Текст пункта оферты: teksty/oferta-dopolnenie-zso-2026-09-22.md", "wait"),
    ("Окно без VPN: локальный прокси через Wi‑Fi работает — НСПД, торги и mos.ru идут с российского адреса при поднятом туннеле", "решение Игоря 22.09 с рамкой: только localhost, только .ru/.рф и явный список, остальным — 403; Chrome через PAC в отдельном профиле; журнал — хост и код; bash scripts/en0_proxy.sh stop выключает. Туннель поднимало само приложение hidemy.name (перезапуск 11:46, «подключаться при запуске»). Проверено живым поиском по адресу и профилем. После перезагрузки прокси запускать снова: bash scripts/en0_proxy.sh start", "go"),
    ("Внести рамку прокси в CLAUDE.md своей рукой (Игорь): фильтр среды не дал Code писать в файл инструкций", "текст правила — в отчёте, раздел 10, и в заголовке scripts/en0_proxy.py; решить также: делать ли прокси агентом launchd (сейчас — фоновый процесс, гибнет с перезагрузкой) и переводить ли сторож/исполнитель заказов на прокси", "wait"),
