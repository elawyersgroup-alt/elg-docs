...
PENDING = [
    ...
    ("lead.php всё ещё шлёт через мёртвый mail(): письма по чек-листу не уходят", "о новых строках leads.csv сообщает только сторож на Маке. Перевод lead.php на проверенную отправку — три строки; запрет «не трогать lead.php» снимает Игорь", "wait"),
    ...
    ("Оба канала подтверждены пробными заказами — снимать ли ручной просмотр orders.csv, решает Игорь", "условие Игоря от 21.09 («пока не заработает хотя бы один») выполнено: письмо — 2 с, окно сторожа — до 5 мин. leads.csv письмами не покрыт — только сторож, пока lead.php на mail()", "wait"),
    ("Удалить семь тестовых строк в orders.csv", "удаляет Игорь через панель; проще удалить файл целиком — order.php создаст его заново с заголовком. Сторож это переживает: узнаёт строки по отпечатку, а не по счёту", "wait"),
