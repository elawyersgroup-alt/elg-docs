...
PENDING = [
    ...
    ("Исполнитель заказов работает: от номера с сайта до готового PDF — без рук, кроме VPN", "сторож кладёт номер в очередь, исполнитель ждёт российский IP, запускает snesut.py --live, кладёт рядом сопроводительный текст Игоря, дописывает строку учёта, показывает окно «Отчёт готов». За Игорем: VPN, взгляд на отчёт, сверка оплаты, отправка PDF. Живой сбор с НСПД до первого заказа не проверен — российского IP 21.09 не было", "go"),
    ...
    ("Сайт: кнопка на странице аудита ведёт сразу на форму заказа", "audit-pered-pokupkoy.html → /proverka-izyatie-krt.html#zakaz (elg-site c6f6918), живая страница совпадает с репозиторием по SHA-256. Подпись «Подробнее о проверке» осталась прежней, хотя ведёт теперь на форму — новую подпись даёт Игорь. Первый экран главной не трогаем: он продаёт линию с оплаченной историей", "go"),
    ("Решить: индексация — какой путь для 13 страниц, убранных Яндексом", "11 страниц серии osnovanie-p… по 206–371 слову своего текста и по одной входящей ссылке. А — влить в хаб с 301 и якорями; Б — дописывать по существу (текст юриста); В — оставить и усилить перекрёстные ссылки. Рекомендация Code: В сейчас, А — если за 3–4 недели страницы не вернутся. Цифры: otchety/analiz-sayta-i-kanalov-2026-09-21.md, раздел 7", "wait"),
    ("Утвердить: ответы бота на четыре вопроса", "цена, срок, что входит, участок не в Москве — собраны из формулировок страниц, у каждой фразы указан источник: teksty/bot-otvety-2026-09-21.md. Предложено подключать кнопками, а не словами-триггерами: слова «срок» и «сколько стоит» пишут и клиенты воронки Росреестра. В бота не вставлено", "wait"),
    ("Директ: автотаргетинг прижат до предела, который даёт кабинет", "целиком выключить нельзя — переключатель заблокирован, без категорий группа не сохраняется. Оставлена одна категория «Целевые запросы», ставка автотаргетинга 0,30 ₽ вместо 60 ₽; фразы по 60 ₽. Оговорка Code: у фразы приоритет перед автотаргетингом, поэтому его показы, скорее всего, не перейдут к фразам, а пропадут. На вечер 21.09: 22 показа, 1 клик за 76,46 ₽, отказ. Вернуть: ставка 60 ₽, галочка «Узкие»", "go"),
