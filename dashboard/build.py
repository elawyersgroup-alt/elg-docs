...
PENDING = [
    ...
    ("ГПЗУ из лота по Брусилова скачивается в первом же российском окне", "решение Игоря 21.09: вместе с пересборкой индекса. Строка get: стоит первой в out/okno_queue.txt, сценарий окна скачает файл сам (только официальные узлы торгов и mos.ru, при сбое строка остаётся в очереди). Это не просто раздел 6: впервые появится измеренное расхождение между книгой 2017 года, реестром и документом органа — цифра, которая заменит снятое «41%» и пойдёт в ТЗ и на страницу Градпрофиля как факт. Выписка ЕГРН и стартовые условия из того же лота — в очереди под решёткой, ждут слова", "late"),
